...
/**
 * Authors: Sangha (Noel) Jang and Andrew Cheng
 * Date: 4/25/22
 * Purpose: PSET 2
...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// if tree is null make new tree w dot as center point
			if (tree == null) {
			...
			// TODO: YOUR CODE HERE
			// if tree is not null, find the nearest point
			if (tree != null) {
				found = tree.findInCircle(x, y, mouseRadius);
			}
			// checks if tree is null, gives message
			if (tree == null) {
	...
	private void test0() {
		...
		System.out.println("All points: " + tree.allPoints());
		int bad = 0;
	...
	/**
	 * tests new tree written by Sangha Jang
	 */
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(250,250), 0,0,800,600); //A
		tree.insert(new Dot(300,300)); //B
		tree.insert(new Dot(50, 50)); //C
		tree.insert(new Dot(400, 400)); //D
		tree.insert(new Dot(400, 50)); //E
		int bad = 0;
		bad += testFind(300,300,10,5,3,1);
		bad += testFind(50,50,10,4,2,1);
		bad += testFind(400,400,10,5,3,1);
		bad += testFind(400,50,10,4,2,1);
	 ...
	 * @param key character on the keyboard
	 */
	@Override
