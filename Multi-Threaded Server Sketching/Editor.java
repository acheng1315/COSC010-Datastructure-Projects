...
public class Editor extends JFrame {	
	...
	/**
	 * Setter for the sketch instance variable
	 */
	...
	private void handlePress(Point p) {
		...
		if (mode == Mode.MOVE) {
			moveFrom = p;
			// loops through each shape in the shape map
			for (int id : sketch.shapes.descendingKeySet()) {
		...
		if (mode == Mode.RECOLOR) {
			for (int key : sketch.shapes.descendingKeySet()) {
				Shape shape = sketch.shapes.get(key);

				if (shape.contains((int)p.getX(), (int)p.getY())) {
					String data = "RECOLOR\n";
					data += key + " " + color.getRGB();
					comm.send(data);
					break;
				}
			}
	...
	private void handleDrag(Point p) {
		...
		if(mode == Mode.DRAW) {
			...
			if (shapeType == "ellipse") {
				...
				((Polyline) curr).update((int) p.getX(), (int) p.getY());
			}
			else if (shapeType == "rectangle") {
	...
	private void handleRelease() {
		...
		if (mode == Mode.DRAW) {
			String data = "DRAW\n";
			data += curr.toString() + " " + curr.getColor().getRGB();
			comm.send(data);
			curr = null;
		}
		...
		if (mode == Mode.MOVE && curr!=null) {
			String data = "MOVE\n";
			data += movingId + " " + curr + " " + curr.getColor().getRGB();
			comm.send(data);
			curr = null;
		}
		repaint();
